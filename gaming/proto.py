...
def main(stdscr):
	...
	pad=curses.newpad(int(lines/2),int(cols/4))
	pady,padx=pad.getmaxyx()
	menu=Menu(pad,pady,padx,stdscr,lines,cols)
	...
	while (c != 'a'):
		stdscr.clear()
		stdscr.refresh()
		if c=='KEY_UP' and player.y > 0 and player.paused==False:
			player.y-=1
		elif c=='KEY_DOWN' and player.y < lines-1 and player.paused==False:
			player.y+=1
		elif c == 'KEY_LEFT' and player.x > 0 and player.paused==False:
			player.x-=1
		elif c=='KEY_RIGHT' and player.x < cols-1 and player.paused==False:
			player.x+=1
		elif c=='c':
			for t in trees:
				if t.player_near:
		...
		# position or to drop wood in chest
			if chest.player_near and player.inventory > 0:
				...
				player.inventory -= 1
		elif c== 'KEY_DOWN' and menu.slider < pady-7:
			menu.slider+=1
		elif c=='KEY_UP' and menu.slider > 2:
			menu.slider-=1
		elif c == 'm':
		...
		try:
			menu.display('something','hello world')
